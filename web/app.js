...
function displayResults(levels) {
    ...
    const stopsHtml = levels.stops.map(stop => `
        <div class="level-item">
            <div class="level-type">${stop.type}</div>
            <div class="level-price stop">$${stop.price.toLocaleString()}</div>
    ...
    const targetsHtml = levels.targets.map((target, i) => `
        <div class="level-item">
            <div class="level-type">T${i + 1} (${target.exit_percentage.toFixed(0)}% exit)</div>
            <div class="level-price target">$${target.price.toLocaleString()}</div>
    ...
    const positionHtml = `
        ...
        <div class="level-item">
            ...
            <div class="level-price">${levels.risk_reward_ratio.toFixed(1)}R</div>
        </div>
    `;
...
function formatLevelsForCopy(levels) {
    ...
    levels.targets.forEach((target, i) => {
        text += `  T${i + 1}: $${target.price.toLocaleString()} (${target.exit_percentage.toFixed(0)}% exit)\n`;
    });
    
    text += `\n📊 POSITION:\n`;
    text += `  Size: ${levels.position_size.toFixed(4)}\n`;
    text += `  Risk: $${levels.risk_amount.toLocaleString()}\n`;
    text += `  R:R: ${levels.risk_reward_ratio.toFixed(1)}R\n`;
